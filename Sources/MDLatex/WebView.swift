...
struct WebView: UIViewRepresentable {
    ...
    /// Called when an image is tapped, providing the image URL.
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        // Register the coordinator for "imageTapped" messages from JS
            ...
            if message.name == "contentHeight", let height = message.body as? CGFloat {
                ...
                DispatchQueue.main.async {
                    ...
                    if self.parent.isAnimationEnabled {
                        ...
                        self.parent.contentHeight = height
                        self.parent.webViewRef.setNeedsLayout()
                    }
                }
            } else if message.name == "imageTapped", let imageUrl = message.body as? String {
                print("Image tapped with URL:", imageUrl)
                DispatchQueue.main.async {
                    self.parent.onImageTapped?(imageUrl)
                }
            }
        }
    }
