...
struct MarkdownLatexParser {
    ...
    static func restoreLatexSegments(into html: String, latexSegments: [String]) -> String {
        ...
        for (index, segment) in latexSegments.enumerated() {
            let originalPlaceholder = "<<<LATEX_\(index)>>>"
            let escapedPlaceholder = originalPlaceholder
                .replacingOccurrences(of: "<", with: "&lt;")
                .replacingOccurrences(of: ">", with: "&gt;")
            
            // Try to replace both the original and escaped versions of the placeholder
            // This handles cases where placeholders are in HTML content (like table cells)
            result = result.replacingOccurrences(of: escapedPlaceholder, with: segment)
            result = result.replacingOccurrences(of: originalPlaceholder, with: segment)
        }
